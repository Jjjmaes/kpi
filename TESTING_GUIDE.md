...
2. 创建项目：
   ...
   **方式一：使用报价明细（精确录入）**
   ...
   - 添加明细2：
     ...
     - 单价：100
     - 金额：自动计算为500
   - 检查：总字数自动计算为10000，总金额自动计算为1000
   - 检查：源语种和目标语种字段自动隐藏
   - 检查：项目级别的字数、单价、总金额字段自动填充并变为只读
...
**预期结果：**
- 项目创建成功
- 如果使用报价明细：明细数据正确保存，总字数和总金额自动计算
- 如果使用报价明细：源语种和目标语种从明细中自动提取
...
4. 在弹出的"阶段性交付"弹窗中：
   - 填写交付说明（可选）
   - 上传一个或多个测试附件（总大小 ≤ 20MB）
   - 点击"确认交付"

**预期结果：**
- 对应项目状态变更为 `translation_done` / `review_done` / `layout_done`
- 项目经理邮箱收到阶段性交付邮件，邮件中包含交付说明和所有附件
- 未上传附件时，只更新状态，不发送邮件
...
**预期结果：**
- 项目状态保持不变（例如仍为"进行中"或某个阶段完成状态）
- 项目创建人（销售）的邮箱收到内部交付邮件，附件完整
- 系统未生成新的 KPI 记录（仅项目完成后才计算 KPI）
...
1. 使用销售账号登录
2. 创建项目，填写报价明细：
   - 添加明细1：文件A.docx，英语→中文，5000字，单价100
   - 添加明细2：文件B.docx，英语→法语，3000字，单价120
   - 添加明细3：文件C.docx，英语→德语，2000字，单价110
3. 检查自动计算：
   - 总字数应为10000（5000+3000+2000）
   - 总金额应为1060（500+360+200）
   - 平均单价应为106（1060/10）
4. 检查字段状态：
   - 源语种和目标语种字段应自动隐藏
   - 字数、单价、总金额字段应自动填充并变为只读
5. 提交项目

**预期结果：**
- 明细数据正确保存
- 总字数和总金额自动计算正确
- 源语种和目标语种从明细中自动提取（英语、中文、法语、德语）
- 项目创建成功
...
#### 测试 10.3：报价单导出（使用明细数据）

1. 创建包含报价明细的项目
2. 在项目详情页面点击"导出报价单"
3. 下载并打开Word文件
4. 检查报价单内容：
   - 应显示每条明细的详细信息（文件名、源语种、目标语种、字数、单价、金额）
   - 应显示明细汇总（总字数、总金额）
   - 应显示项目基本信息
...
#### 测试 10.4：报价单导出（使用项目级别数据）

1. 创建不包含报价明细的项目（使用项目级别的字数和金额）
2. 在项目详情页面点击"导出报价单"
3. 下载并打开Word文件
4. 检查报价单内容：
   - 应显示项目级别的字数和金额
...
1. 创建项目，填写报价明细：
   - 明细1：文件A.docx，英语→中文，5000字，单价100
   - 明细2：文件A.docx，英语→法语，5000字，单价120
   - 明细3：文件B.docx，英语→德语，3000字，单价110
2. 检查：
   - 总字数应为13000（5000+5000+3000）
   - 总金额应为1480（500+600+330）
   - 目标语种应包含中文、法语、德语
3. 导出报价单，检查是否包含所有语种
